...
class Detection:
    def __init__(self,
                num_doubles,
                num_scintillators,
                num_intra_structure_gaps,
                square_side_len,
                z_per_scintillator,
                space_between_scintillators,
                space_between_top_bottom,
                ):
    ...
    def set_initial_values(self,
        ...
        # Global variables
        self.level_count = 1
        self.n_initial = square_side_len # Sideview length of scintillator in unit x


        # These values are used for x perspective
        self.upper_side_views = [(0, self.n_initial)] # (start, end) coordinates for each level 
        self.lower_side_views = [(0, self.n_initial)]

        self.plate_thickness_initial = z_per_scintillator # In unit x
        self.intra_level_gap_initial = space_between_scintillators #Actual physical gap between each level, in unit x
        self.inter_level_gap_initial = self.plate_thickness_initial + self.intra_level_gap_initial # Adjusted inter level gap for computation 

        self.half_gap_size_initial =  space_between_top_bottom/2# In unit x
        self.top_half_gap_initial = self.half_gap_size_initial + self.plate_thickness_initial + self.intra_level_gap_initial
        self.bottom_half_gap_initial = self.half_gap_size_initial
        self.gap_line = 0
        self.highest_point_initial = self.half_gap_size_initial + num_intra_structure_gaps*self.intra_level_gap_initial + num_scintillators*self.plate_thickness_initial # Values custom set to this detector
        self.lowest_point_initial = -self.highest_point_initial

    def reset_to_initial_values(self):
        # Global variables
        self.level_count = 1
        self.n = self.n_initial # Sideview length of scintillator in unit x
        ...
        self.plate_thickness = self.plate_thickness_initial # In unit x
        self.intra_level_gap = self.intra_level_gap_initial #Actual physical gap between each level, in unit x
        self.inter_level_gap = self.inter_level_gap_initial # Adjusted inter level gap for computation 

        self.half_gap_size =  self.half_gap_size_initial # In unit x
        self.top_half_gap = self.top_half_gap_initial
        self.bottom_half_gap = self.bottom_half_gap_initial
        self.gap_line = 0
        self.highest_point = self.highest_point_initial # Values custom set to this detector
        self.lowest_point = -self.lowest_point_initial
